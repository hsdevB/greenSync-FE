...
async function askSmartFarmBot(userMessage) {
    // LLM 프롬프트
  const fullPrompt = `
...
- Please tell me the humidity unit in (%)

IMPORTANT: When analyzing farm data, always include specific numerical values:
- Temperature: Always include the value with °C (e.g., "온도: 22°C")
- Humidity: Always include the value with % (e.g., "습도: 65%")
- Harvest prediction: Always include the value with kg (e.g., "다음주 수확량: 15kg")
...
답변:
  ...
  try {
    ...
    return clean(response.data.response);
  } catch (err) {
    console.error('LLM 호출 오류:'  , err.message);
    throw new Error('LLM 서버 호출 중 오류 발생');
  }
}

export { askSmartFarmBot };
