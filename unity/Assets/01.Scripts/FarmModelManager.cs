...
public class FarmModelManager : MonoBehaviour
{
    ...
    private GameObject currentFarmModel;
    ...
    public void InitializeFarm(string farmDataJson)
    {
        try
        {
            ...
            // 기존 농장 모델 제거
            if (currentFarmModel != null)
            {
                DestroyImmediate(currentFarmModel);
            }

            // 농장 타입에 따른 모델 로드
            LoadFarmModel(farmData.farmType, farmData.houseType);
    ...
    private void LoadFarmModel(string farmType, string houseType)
    {
        ...
        {
            // 농장 모델 인스턴스화
            currentFarmModel = Instantiate(prefabToLoad, Vector3.zero, Quaternion.identity);
            currentFarmModel.name = $"Farm_{currentFarmData.farmId}";

            // 농장 모델 로드 후 MessageManager의 참조 업데이트
            UpdateMessageManagerReferences();
        }
        else
        {
            Debug.LogError($"No prefab found for farm type: {farmType} + {houseType}");
        }
    }
